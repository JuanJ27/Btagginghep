...
def classical_controls(seed: int) -> dict:
    return {
        ...
        "rbf_svm": make_pipeline(StandardScaler(), SVC(kernel="rbf", probability=True, class_weight="balanced", random_state=seed)),
...
def build_angle_encoded_vqc(maxiter: int = 50):
    ...
    return VQC(feature_map=feature_map, ansatz=ansatz, sampler=sampler, optimizer=COBYLA(maxiter=maxiter))
...
def fit_angle_vqc(
    ...
    """Fit angular scaling on b/c training rows only, then train and score a local VQC."""
    ...
    model = build_angle_encoded_vqc(maxiter=maxiter)
    model.fit(angular_train, y_train)
    probabilities = np.asarray(model.predict_proba(angular_test), dtype=float)
    if probabilities.ndim != 2 or probabilities.shape[1] != 2:
        raise HybridContractError("VQC predict_proba must return b/c probability columns.")
    return model, preprocessor, angular_validation, probabilities[:, 1]
